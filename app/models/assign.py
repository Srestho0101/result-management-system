...
class Department(db.Model):
    ...
    department_code = db.Column(db.Integer,nullable=False)
    department_name = db.Column(db.String(150),nullable=False)
    principal_id = db.Column(db.Integer,db.ForeignKey("principal_info.principal_id"),nullable=False)
...
class Subjects(db.Model):
    ...
    principal_id = db.Column(db.Integer,db.ForeignKey("principal_info.principal_id"),nullable=False)
    student_id = db.Column(db.Integer,db.ForeignKey("student_data.student_id"))
    teacher_id = db.Column(db.Integer,db.ForeignKey("teacher_info.teacher_id"))
    __table_args__ = (
        db.UniqueConstraint(
            "principal_id",
...
class Curriculum(db.Model):
    ...
    subject = db.relationship( "Subjects", lazy=True)
...
class TeacherAssignment(db.Model):
    ...
    teacher = db.relationship("TeacherAddInfo",lazy=True)
    department = db.relationship("Department",lazy=True)
    subject = db.relationship( "Subjects",lazy=True)